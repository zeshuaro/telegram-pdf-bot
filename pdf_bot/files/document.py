...
from pdf_bot.constants import DECRYPT, ENCRYPT, ROTATE, SCALE, SPLIT, PREVIEW, \
    TO_PHOTO, EXTRACT_PHOTO, RENAME, CROP, CANCEL, WAIT_DOC_TASK
from pdf_bot.utils import set_lang


def ask_doc_task(update, context):
    _ = set_lang(update, context)
    keywords = sorted([_(DECRYPT), _(ENCRYPT), _(ROTATE), _(SCALE), _(SPLIT),
                       _(PREVIEW), _(TO_PHOTO), _(EXTRACT_PHOTO), _(RENAME), _(CROP)])
    keyboard_size = 3
