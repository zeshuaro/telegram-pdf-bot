...
from pdf_bot.pdf_processor import OCRPDFProcessor
...
class TestOCRPDFProcessor(
    ...
    def test_process_file_task(self) -> None:
        self.pdf_service.ocr_pdf.return_value.__enter__.return_value = self.FILE_PATH

        with self.sut.process_file_task(
            self.TELEGRAM_DOCUMENT_ID, self.TELEGRAM_TEXT
        ) as actual:
            assert actual == self.FILE_PATH
            self.pdf_service.ocr_pdf.assert_called_once_with(self.TELEGRAM_DOCUMENT_ID)
