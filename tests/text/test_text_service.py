...
def fixture_text_service(
    text_repository: TextRepository, pdf_service: PdfService
) -> TextService:
    return TextService(text_repository, pdf_service)
...
def test_check_text(
    ...
    with patch("pdf_bot.text.text_service.check_user_data") as _check_user_data, patch(
        "pdf_bot.text.text_service.send_result_file"
    ) as send_result_file:
...
def test_check_text_invalid_user_data(
    ...
    with patch("pdf_bot.text.text_service.check_user_data") as check_user_data, patch(
        "pdf_bot.text.text_service.send_result_file"
    ) as send_result_file:
